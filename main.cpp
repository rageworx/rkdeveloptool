...
bool parse_config_file(const char *pConfigFile, CONFIG_ITEM_VECTOR &vecItem)
{
    ...
    char *pConfigBuf = new char[iFileSize + 1];
    if (!pConfigBuf)
    {
        fclose(file);
        return false;
...
bool parse_parameter_file(char *pParamFile, PARAM_ITEM_VECTOR &vecItem, CONFIG_ITEM_VECTOR &vecUuidItem)
{
    ...
    char *pParamBuf = new char[iFileSize];
    if (!pParamBuf) 
    {
        fclose(file);
        return false;
...
bool is_sparse_image(char *szImage)
{
    ...
    u32 uiRead = fread(&head, 1, sizeof(head), file);
...
bool is_ubifs_image(char *szImage)
{
    ...
    u32 magic;
    u32 uiRead = fread(&magic, 1, sizeof(magic), file);
...
void gen_rand_uuid(unsigned char *uuid_bin)
{
    ...
    for ( size_t i = 0; i < sizeof(id) / sizeof(*ptr); i++ )
...
void prepare_gpt_backup(u8 *master, u8 *backup)
{
    ...
    u64 val = le64_to_cpu(gptMasterHead->my_lba);
...
bool get_lba_from_gpt(u8 *master, char *pszName, u64 *lba, u64 *lba_end)
{
    ...
    for ( size_t i = 0; i < le32_to_cpu(gptMasterHead->num_partition_entries); i++) 
    {
        size_t j = 0;
...
bool get_lba_from_param(u8 *param, char *pszName, u32 *part_offset, u32 *part_size)
{
    ...
    bool bRet = parse_parameter((char*)param, vecItem, vecUuid);
    if (!bRet)
        return false;
...
bool mergeBoot(void) 
{
    ...
    snprintf( version, MAX_LINE_LEN, "%s", gSubfix);
...
{
    UINT uiRead = fread(&chunk, 1, sizeof(chunk_header), file);
...
{
    UINT uiRead = fread(pBuf, 1, dwSize, file);
    if (uiRead!=dwSize)
    {
        if (g_pLogObject)
        {
...
#else
    // Windows may placed config file with rkdeveloptool.exe.
...
#else
    // Windows log file may placed "log" directory with rkdevleoptool.exe
    if (opendir(strLogDir.c_str()) == NULL)
