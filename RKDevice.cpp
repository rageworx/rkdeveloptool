...
bool CRKDevice::EraseEmmc()
{
    ...
    UINT uiTotalCount = uiCount = m_flashInfo.uiFlashSize*2*1024;
    uiSectorOffset = 0;
    DWORD dwLayerID;
...
{
    int iRet = m_pComm->RKU_DeviceRequest(requestCode, pBuffer, dwDataSize);
...
{
    BYTE data[8] = {0};
    int ret = m_pComm->RKU_ReadCapability(data);
    if (ret != ERR_SUCCESS)
    {
        if (m_pLog)
        {
