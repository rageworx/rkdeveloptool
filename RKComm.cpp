...
bool CRKUsbComm::InitializeUsb(STRUCT_RKDEVICE_DESC devDesc)
{
    ...
    for( size_t i = 0; i < pConfigDesc->bNumInterfaces; i++) 
    {
        pInterface = pConfigDesc->interface + i;
        
        for( size_t j = 0; j < pInterface->num_altsetting; j++) 
        {
            ...
            for( size_t k = 0; k < pInterfaceDesc->bNumEndpoints; k++ ) 
            {
                ...
                {//found it
                    ...
                    printf("\n\tm_pipeBulkIn=0x%02X, m_pipeBulkOut=0x%02X", m_pipeBulkIn, m_pipeBulkOut );
                    printf("\n\tSize of EndPoint Descriptor : %d",pEndpointDesc->bLength);    
                    printf("\n\tType of Descriptor : %d",pEndpointDesc->bDescriptorType);    
                    printf("\n\tEndpoint Address : 0x0%x",pEndpointDesc->bEndpointAddress);    
                    printf("\n\tMaximum Packet Size: %x",pEndpointDesc->wMaxPacketSize);    
                    printf("\n\tAttributes applied to Endpoint: 0x%02X",pEndpointDesc->bmAttributes);    
