...
{
    int  nRead = 0;
    int iRet = libusb_bulk_transfer((libusb_device_handle *)m_pUsbHandle, m_pipeBulkIn, lpBuffer, dwSize, &nRead, 5000);
    if (iRet != 0) 
    {
        if (m_log) 
        {
...
int CRKUsbComm::RKU_ReadLBA(DWORD dwPos, DWORD dwCount, BYTE* lpBuffer, BYTE bySubCode)
{
    ...
    DWORD dwTotal = usSectorLen * wSectorSize;
