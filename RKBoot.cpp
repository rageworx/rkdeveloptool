...
bool CRKBoot::SaveEntryFile(ENUM_RKBOOTENTRY type,UCHAR ucIndex,string fileName)
{
	...
	PSTRUCT_RKBOOT_ENTRY pEntry = (PSTRUCT_RKBOOT_ENTRY)(m_BootData+dwOffset+(ucSize*ucIndex));
...
char CRKBoot::GetIndexByName(ENUM_RKBOOTENTRY type,char *pName)
{
	...
	{
		PSTRUCT_RKBOOT_ENTRY pEntry = (PSTRUCT_RKBOOT_ENTRY)(m_BootData+dwOffset+(ucSize*i));
...
CRKBoot::CRKBoot(PBYTE lpBootData,DWORD dwBootSize,bool &bCheck)
{
	...
	if (lpBootData!=NULL)
	{
		...
		PSTRUCT_RKBOOT_HEAD pBootHead = (PSTRUCT_RKBOOT_HEAD)(m_BootData);
